...
vid_captured = cv.VideoCapture("test2.mp4")
while(vid_captured.isOpened()):
    _, frame = vid_captured.read()
    ...
    blended_image = cv.addWeighted(frame, 0.8, line_image, 1, 1) # adds the pixels values - hence the reason as why lines were generated on the black image
